...
public class ObsdYVsWeightedPredYLinearPlotGenerator extends
		JInternalFrame {

	PkPdInfo pkPdInst;
	
	public ObsdYVsWeightedPredYLinearPlotGenerator(double[] xPlot,
		...
		pkPdInst.getWorkSheetOutputInst().getPlotOutputs().getLinearPlotsForObsevedYVsWeightedPredictedY().add(chart);
	...
	private JFreeChart createChart(XYDataset dataset, String profile) {
		...
		final JFreeChart chart = ChartFactory.createXYLineChart(profile, // chart
																			// title
				"Observed "
						+ pkPdInst.xColumnName + "(" + concMassUnit
