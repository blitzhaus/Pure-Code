...
public class PkPdLinkLibModelDefiner {
	...
	public double choosePkPdModel(double[] par, double[] infusionTime,
	...
	private double func_diff1(double[] par, double[] infusionTime,
			...
			throws RowsExceededException, WriteException, BiffException,
			IOException {
		EffectSiteConcCalculation effectSiteConcCalInst = new EffectSiteConcCalculation();

		double[] fn = new double[1];
		double Emax = par[0];
		double EC50 = par[1];
		double Ke0 = par[2];
		double c = effectSiteConcCalInst.chooseLibraryModel(Ke0, infusionTime, conDose, conDosingTime, x, fn_no, Extra_DATA, rowIdx, row);
