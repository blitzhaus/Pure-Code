...
public class HybridPSODriver {
	...
	*/
	...
	public void performCoMutPSO(int startIdx, int popSize, int descCount,
		...
		procInputInst = pkPdInfoInst.copyProcessingInput();
	...
	void performPSOProc(int firstIdx, int secIdx, int descCount, double[][] X,
		...
		for (int i = 0; i < lowerBound.length; i++) {
			double[] temp1 = ind1.getParameters();
			double[] temp2 = ind2.getParameters();
			if(temp1[i] < 0 || temp2[i] <0)
			{
				System.out.println("negative param value");
			}
		}
		...
		//if (Math.random() < psoProcProb) // for individual - 1
		{
			...
			descrs1 = applyConstratint(descrs1, lowerBound, upperBound);
		...
		//if (Math.random() < psoProcProb) // for individual - 2
		{
			...
			descrs2 = applyConstratint(descrs2, lowerBound, upperBound);
	...
	private double[] applyConstratint(double[] descrs1, double[] lowerBound, double[] upperBound) {
		Random rand = new Random(99999);
		for (int i = 0; i < descrs1.length; i++) {
			if(descrs1[i] <= lowerBound[i] || descrs1[i] >upperBound[i])
			{
				double max = upperBound[i];
				double min = lowerBound[i];
				descrs1[i] = rand.nextFloat() * (max - min) + min;
			}
		}
