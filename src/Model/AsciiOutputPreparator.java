...
public class AsciiOutputPreparator {

	public AsciiOutputPreparator() {
		...
		for (int i = 0; i < x[0].length; i++) {
			...
			for (int r = 0; r < pkPdInst.row[i]; r++) {
				...
				dataStructInst.calcYval[r] = asciiEvluationInst.evaluateCode(
						parameter, x1, i, profileNo, false);
	...
	public void misOutpuAndSecondParamCal(double[] parameter, double[][] x,
		...
		secondaryParamCalInst.calculateSecondaryParameter(pkpdInst.parameter,
				pkpdInst.modelNumber, dataStructInst.dose[0], A_inv, SD);
	...
	public void calculateWeightedCorrObsYAndPredY(double[] x, double[] y) {
		...
		double Ra = 0;
		// .println("########### In correlation coefficient calculation");
		int row = x.length;
