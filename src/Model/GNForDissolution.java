...
public class GNForDissolution {
	...
	public double[] gaussNewtonLM(int modelNumber, double[] initial,
			...
			String[][] dataSortVariables, int noOfParam, double pdInc) throws RowsExceededException, WriteException,
		...
		while (IT <= pkpdInstance.iter) {
			...
			double RSS_old = Objective(parameter, infusionTime, dose, dosingTime, X, Y, Extra_DATA, row);
			...
			System.out.println("iter no" + IT);

			RSS_new = Objective(finalBeta, infusionTime, dose, dosingTime, X, Y, Extra_DATA, row);
	...
	public double[] GaussNewtonLH(int modelNumber, double[] initial,
			...
			double pdInc) throws RowsExceededException, WriteException,
			BiffException, IOException {
		int Npar = initial.length;
		double mu_min = 0;
		double[] parameter = new double[Npar];
		...
		while (IT <= pkpdInstance.iter) {
			...
			double WRSS0 = Objective(parameter,infusionTime,dose,dosingTime, X, Y, Extra_DATA, row);
			...
			double WRSS05 = Objective(Beta05, infusionTime, dose, dosingTime, X, Y, Extra_DATA, row);
			...
			double WRSS1 = Objective(Beta1, infusionTime, dose, dosingTime, X, Y, Extra_DATA, row);
			...
			if (WRSS0 < WRSS1) {
				...
				while (WRSS1 > WRSS0) {
					...
					WRSS1 = Objective(Beta1, infusionTime, dose, dosingTime, X, Y, Extra_DATA, row);
				}
			...
			double srs = Objective(finalBeta, infusionTime, dose, dosingTime, X, Y, Extra_DATA, row);
	...
	public double[] HartleyMethod(double[] initial, double[][] X, double[][] Y,
			...
			String[][] dataSortVariables, int noOfParam, double pdInc)
	...
	private double[][] detParameters(double[] par, double[] infusionTime, double[] dose, double[] dosingTime, double[][] X, double[][] Y,
			double[][] Extra_DATA, int[] row, double[] delta)
			throws RowsExceededException, WriteException, BiffException,
			IOException {
		...
		for (int f = 0; f < Nfun; f++) {
			int r = row[f];
			double[][] temp_J = Jacobian(par, infusionTime, dose, dosingTime, X, Y, Extra_DATA, delta, f, row);
			double[][] temp_Q = new double[r][r];
			if (pkpdInstance.ifScaling != 0)
				temp_Q = ScaledWeights(par, infusionTime, dose, dosingTime, X, Y, Extra_DATA, f, row);
			else
				temp_Q = weights(par,infusionTime, dose, dosingTime, X, Y, Extra_DATA, f, row);
		...
		// System.out.println("delta beta value");
		/*
		 * for (int i = 0; i < deltaBeta.length; i++) { //
		 * System.out.println(""+deltaBeta[i][0]); }
		 */
		/*
		 * for(int i = 0; i < Npar; i++) newPar[i] = par[i] + mu *
		 * deltaBeta[i][0]; System.out.println(" after Marquardt correction");
		 * return newPar;
		 */
	...
	double[][] CalculateA_mat(double[] par,double[] infusionTime, double[] dose, double[] dosingTime, double[][] X, double[][] Y,
			double[][] Extra_DATA, int[] row) throws RowsExceededException,
	...
	// Determine B matrix
	private double[][] CalculateB_mat(int modelNumber, double[] par,double[] infusionTime, double[] dose, double[] dosingTime,
			double[][] X, double[][] Y, double[][] Extra_DATA, int[] row)
			throws RowsExceededException, WriteException, BiffException,
			IOException {
		int Npar = par.length;
		double[][] B_mat = new double[Npar][1];
		int Nfun = X[0].length;
		...
		for (int f = 0; f < Nfun; f++) {
			...
			B_mat = sumMatrices(B_mat, temp_B);
		}
		return B_mat;
	}
	...
	public double Objective(double[] par, double[] infusionTime, double[] dose, double[] dosingTime, double[][] X, double[][] Y,
		...
		for (int i = 0; i < Nfun; i++) {
			...
			double[][] weightMat = weights(par, infusionTime,  dose, dosingTime,  X, Y, Extra_DATA, i, row);
	...
	// Objective function: (sum of residuals)
	private double Objective_SRS(double[] par, double[][] X, double[][] Y,
			double[][] Extra_DATA, int[] row) throws RowsExceededException,
			WriteException, BiffException, IOException {
		double SRS = 0;
		/*int Nfun = appInfo.getWorkSheetObjectsAL().get(
				appInfo.getSelectedSheetIndex()).getCaInfo()
				.getProcessingInputs().getProfileAndParamInfoObj()
				.getNumberOfFunction();
		double[] ResSq = new double[Nfun];
		double SRS = 0;
		for (int i = 0; i < Nfun; i++) {
			int r = row[i];
			...
			for (int j = 0; j < r; j++) {
				double resSq = Math.pow(errorMat[j][0], 2);
				ResSq[i] = ResSq[i] + resSq;
			}
	...
	public double[][] ScaledWeights(double[] par, double[] infusionTime, double[] dose, double[] dosingTime, double[][] X, double[][] Y,
			double[][] Extra_DATA, int fn_no, int[] row)
			throws RowsExceededException, WriteException, BiffException,
			IOException {
		// int Nfun = row.length;
		fn_no = 0;
		...
		int r = row[fn_no];
		double[][] scaledWts = new double[r][r];
		double[][] weightMat = weights(par, infusionTime,  dose,  dosingTime,  X, Y, Extra_DATA, fn_no, row);
	...
	private double[][] computeA(double[][] J_mat, double[][] Q_mat)
		...
		/*ApplicationInfo appInfo = pkpdMainInstance.getAppInfo();

		String S = appInfo.getWorkSheetObjectsAL().get(
				appInfo.getSelectedSheetIndex()).getCaInfo()
				.getProcessingInputs().getModelInputTab2Obj()
				.getRoundingOffValue();

		pkpdInstance.roundTo = Integer.parseInt(S);
*/
