...
public class PartialDevtLogPlotGenerator extends JInternalFrame {
	
	PkPdInfo pkPdInst;
	
	public PartialDevtLogPlotGenerator(final String title,
			double[] xplot, double[][] yPlot, String profile,
			int numberOfParameter) throws RowsExceededException, WriteException, BiffException, IOException {
		...
		for (int j = 0; j < numberOfParameter; j++) {
			XYSeries observedData = new XYSeries(
					pkPdInst.paramName[j]);

			for (int i = 0; i < xplot.length; i++) {
				if (yPlot[i][j] > 1)
					observedData.add(xplot[i], yPlot[i][j]);
				else
					continue;
		...
		pkPdInst.getWorkSheetOutputInst().getPlotOutputs().getLogPlotForPartialDerivative().add(chart);
