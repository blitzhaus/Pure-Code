...
public class NCAoutputPlots {
	...
	void GenerateplottsForNCAOutput(double XPlot[], double YPlot[],double[] termX, double[] Ycalc, int lambdaZ, String profile) throws RowsExceededException, WriteException, BiffException, IOException{//JDesktopPane displayResultsinternalFrameDesktopPane 
		...
		generateCorrepondingLogGraph(XPlot,YPlot,termX,Ycalc,lambdaZ,profile);
	}
	...
	/*void generateLogarithmicGraph(double[] observedXArray,
			double[] observedYArray, double[] predictedY,String profile) throws RowsExceededException, WriteException, BiffException, IOException {
		
		GenerateLogGraph logGraphIntenalFrame = new GenerateLogGraph("Log view",observedXArray,observedYArray,predictedY, profile);
	}



	void generateNormalGraph(double[] observedXArray,
			double[] observedYArray, double[] predictedY, String profile) throws RowsExceededException, WriteException, BiffException, IOException {
		GenerateNormalGraph  normalGraphInternalFrmae = new GenerateNormalGraph(observedXArray,observedYArray,predictedY, profile);
	}*/
