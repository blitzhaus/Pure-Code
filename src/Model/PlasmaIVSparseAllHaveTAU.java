...
public class PlasmaIVSparseAllHaveTAU {
	...
	private double calculateLambdaZAndAucRelatedParam(int j,
			double convertedDose) {
		...
		if (ncaInst.noPtsLambdaZ == 0) {
			...
			setLambdaZAndAucRelatedParamToMissiing();

		} else {

			computeLambdaZAndAucRelatedParam(j, convertedDose);
	...
	private void computeLambdaZAndAucRelatedParam(int j, double convertedDose) {
	...
	private void setLambdaZAndAucRelatedParamToMissiing() {
	...
	private void concRelatedParamComputation(int j) {
		ncaInst.Cmax = ncaInst.maxVal(ncaInst.Y);
		ncaInst.Cmax_D = ncaInst.Cmax / ncaInst.Dose[j];
