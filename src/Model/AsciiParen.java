...
public class AsciiParen {

	public static AsciiParen ASCIIPAREN = null;
	public static AsciiParen createAsciiParen(){
		if(ASCIIPAREN == null){
			ASCIIPAREN = new AsciiParen();
		}
		return ASCIIPAREN;
	}
	...
	public void main(String[] args) {
		...
		for(int i = 0; i < str.length() - 1; i++) {
			if(str.charAt(i) == '(' && str.charAt(i + 1) == '(') {
				...
				if(str.charAt(ri + 1) == ')' && ri + 1 == AsciiTreeTraversal.findMatchingRightParen(str, i)) {
...
//				System.out.println("left : " + left);
				...
				if (str.charAt(i - 1) == ')' || str.charAt(i + 1) == '(') {
					if (str.charAt(i - 1) == ')' && str.charAt(i + 1) == '(') {
						int matchLeft = AsciiTreeTraversal.createAsciiTreeTrav().findMatchingLeftParen(str,
								i - 1);
						int matchRight = AsciiTreeTraversal.findMatchingRightParen(str,
								i + 1);
					...
					if (str.charAt(i - 1) == ')') {
						int match = AsciiTreeTraversal.createAsciiTreeTrav()
								.findMatchingLeftParen(str, i - 1);
						match -= 1;
						if (match >= 0
								&& str.charAt(match) == '('
								&& AsciiTreeTraversal
										.findMatchingRightParen(str, match) <= i
										+ right.length()) {
							continue;
						}
						...
						if (match < str.length()
								&& str.charAt(match) == ')'
								&& AsciiTreeTraversal.createAsciiTreeTrav().findMatchingLeftParen(str, match) >= i
										- left.length()) {
							continue;
						}
					}
	...
	public String getRight(String str, int idx) {
		...
		//	System.out.println("iterator : " + iterator + " idx : " + idx + " j : " + j);
			int rig = AsciiTreeTraversal.findMatchingRightParen(str, iterator + idx + 1);
