...
public class EffectSiteConcCalculation {
	...
	public double chooseLibraryModel(double par, double[] infusionTime,
		...
		if(ifSimulation == true)
		{
			...
			linkParam = simulationInst.paramForLinkModel;
			plasmaConc = simulationInst.plasmaConc[rowIdx];
			
		}else
		{
			dataStructInst = ListOfDataStructures.createListOfDataStructInstance();
			linkParam = dataStructInst.paramForLinkModel;
			plasmaConc = dataStructInst.concForLinkModel[rowIdx][0];
		...
		value = choosePkModel(linkParam, infusionTime, conDose, conDosingTime, x,
	...
	public double func_diff1(double[] par, double[] infusionTime,
		...
		for (int j = 0; j <= ndose; j++) {
			...
			amt = (Dose*Ke0/(V*(Ke0 - K10))) * (Math.exp(-1.0 * K10 * tr) - Math.exp(-1.0 * Ke0 * tr));
			res = Math.max(0, amt);
			fn[0] = fn[0] + res;
		...
		for (int j = 0; j <= ndose; j++) {
			...
			amt = (Dose / Tinf)*Ke0
					...
					* (Math.exp(-1.0 * K10 * Tstar) - Math.exp(-1.0 * K10 * tr))
					+(Dose / Tinf)*Ke0
					* (1 / (V * Ke0*(K10 - Ke0)))
					* (Math.exp(-1.0 * Ke0 * Tstar) - Math.exp(-1.0 * Ke0 * tr));
			res = Math.max(0, amt);
			fn[0] = fn[0] + res;
	...
	public double func_diff3(double[] par, double[] infusionTime,
		...
		for (int j = 0; j <= ndose; j++) {
			...
			if (tr > 0) {
				amt = K01*Ke0
						* (Dpo/V_f)
						* (Math.exp(-1.0 * K10 * tr)/((K01-K10)*(Ke0 - K10)) + Math
								.exp(-1.0 * K01 * tr)/((K10-K01)*(Ke0 - K01))
								+  Math
								.exp(-1.0 * Ke0 * tr)/((K10-Ke0)*(K10 - Ke0)));
				res = Math.max(0, amt);
				fn[0] = fn[0] + res;
			}
	...
	public double func_diff4(double[] par, double[] infusionTime,
		...
		for (int j = 0; j <= ndose; j++) {
			...
			amt = K01*Ke0
			* (Dpo/V_f)
			* (Math.exp(-1.0 * K10 * (tr-t_lag))/((K01-K10)*(Ke0 - K10)) + Math
					.exp(-1.0 * K01 * (tr-t_lag))/((K10-K01)*(Ke0 - K01))
					+  Math
					.exp(-1.0 * Ke0 * (tr-t_lag))/((K10-Ke0)*(K10 - Ke0)));
			res = Math.max(0, amt);
			fn[0] = fn[0] + res;
	...
	public double func_diff7(double[] par, double[] infusionTime,
		...
		for (int j = 0; j <= ndose; j++) {
			...
			alpha = (K12 + K21 + K10 + Math.sqrt(Math.pow(K12 + K21 + K10, 2)
					- 4 * K21 * K10)) / 2;
			A = (Ke0 * Div / V1) * ((K21 - alpha) / ((Ke0 - alpha)*(beta - alpha)));
			B = (Ke0 * Div / V1) * ((K21 - beta) / ((Ke0 - beta)*(alpha - beta)));
			C = (Ke0 * Div / V1) * ((K21 - Ke0) / ((alpha-Ke0)*(beta - Ke0)));
	...
	public double func_diff8(double[] par, double[] infusionTime,
		...
		for (int j = 0; j <= ndose; j++) {
			...
			double A2 = (Ke0 / V1) * ((K21 - alpha) / ((Ke0 - alpha)*(beta - alpha)));
			double B2 = (Ke0 / V1) * ((K21 - beta) / ((Ke0 - beta)*(alpha - beta)));
			double C2 = (Ke0 / V1) * ((K21 - Ke0) / ((alpha-Ke0)*(beta - Ke0)));
	...
	public double func_diff9(double[] par, double[] infusionTime,
		...
		for (int j = 0; j <= ndose; j++) {
			...
			A = (Ke0*Dpo / (Tinf * V1)) * (K21 - alpha) / ((alpha - beta) * alpha);
