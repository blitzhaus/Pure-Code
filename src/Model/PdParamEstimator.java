...
public class PdParamEstimator {
	
	

	static PdParamEstimator pdParamCalInst = null;

	public static PdParamEstimator createPdParamCalInstance() {
		if (pdParamCalInst == null) {
			pdParamCalInst = new PdParamEstimator();
		}
		return pdParamCalInst;
	}
	...
	public void calculatePdParameters(ApplicationInfo appInfo) throws RowsExceededException, WriteException, BiffException, IOException
	{
		
		
		pkPdInst = PkPdInfo.createPKPDInstance();
		pkPdInst.analysisType = "pd";
		...
		pkPdInst.defaultUnitForCA = procInputInst.getParameterUnitsDataObj().getDefaultUnitArray();

		pkPdInst.preferredUnitForCA = procInputInst.getParameterUnitsDataObj().getPreferredUnitsArray();
		
		pkPdInst.modelNumber =  procInputInst.getModelInputTab1Obj().getModelNumberForCA();
		...
		}else
		{
			...
			profileRelatedInfoInst.copyProfileRelatedInfo(pkPdInst.analysisType);
			...
			modelEnginInst.copyModelAndEngineSettingsInfo(pkPdInst.analysisType);
			...
			//double convertAmount = dosingInfoInst.doseToConcMassUnitConvertAmount();
			...
			.add(
					StringUtils
							.rightPad(
									"FINAL EXECUTION (OPTIMUM WEIGHTING SCHEME) "
											,
									80)
							+ "\r\n\r\n");
...
private void determineOptimalWeighting() {
	...
	private void mainParamComputationLoop() throws RowsExceededException, WriteException, BiffException, IOException {
		int sum = 0;
		...
		for (int profileNo = 0; profileNo < dataStructInst.noOfSubjects; profileNo++) {
			...
			profileRelatedInfoInst.createSingleProfileData(pkPdInst.analysisType, sum, profileNo);
			...
			if (ifProfileIsInsufficient == true) {
				...
				if (pkPdInst.exceptionForCurrentProfile == 0) {
					if (procInputInst.getModelInputTab2Obj()
						...
						initParamInst.retrievingInitialParamValInfo(pkPdInst.analysisType, profileNo);
