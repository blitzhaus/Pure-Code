...
public class RestoreMM {
	...
	public RestoreMM(String[] pathSplits) {
		...
		LogEntry.createLogEntryInstance().logEntry("Restored Michaelis-Menten");
		...
		DDViewLayer.createMTInstance().mainTabbedFrame
				.setTitle("Michaelis-Menten Analysis");
		...
		try {
			...
			MmMainScreenCreator mmMainScreen = MmMainScreenCreator
					.createMmMainScreenInstance();
		...
		for (int i = 0; i < appInfo
			...
			CaResultDispCompCreator.createCaResDispCompInst().completeTextOutputTextArea
					.append(appInfo
							...
							.getMmInfo().getWorkSheetOutputs().getTextoutput()
							.get(i));
