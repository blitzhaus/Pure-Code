...
import view.CaMapingHandler;
import view.DeConvoLambdaZCalculation;
import view.DeConvoModelOptionsGui;
import view.DisplayContents;
import view.ProcessingInputsInfo;



public class DeConvoCoreLambdaZCal {



	public static DeConvoCoreLambdaZCal CORE_LAMBDAZ_INST = null;

	public static DeConvoCoreLambdaZCal createCoreLamInst() {
		if (CORE_LAMBDAZ_INST == null) {
			CORE_LAMBDAZ_INST = new DeConvoCoreLambdaZCal();
		}
	...
	DeConvoLambdaZCalculation lambdaZInst;
	...
	public void lambdaZCalculation(double[] time, double[] conc,
		...
		lambdaZInst = DeConvoLambdaZCalculation.createLambdaZInstance();
		
		initialiZVar();
		
		appInfo = DisplayContents.createAppInfoInstance();
		routeOfAdmin = procInputInst.getModelInputsObj()
				.getRootOfAdmistration();
		
		if(routeOfAdmin == 3)
		{
			 try{
				 routeOfAdmin = procInputInst
		...
		weightingIndex = DeConvoModelOptionsGui.createDeConvoOptionsInstance().weightingOptions
				.getSelectedIndex();
