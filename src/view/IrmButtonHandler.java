...
public class IrmButtonHandler {
	...
	public void setAllTheGuiScreenObjectsToNull()
		...
		IrmMainScreenCreator.IRM_MAIN_SCREEN = null;
		IrmMainScreenGUICreator.IRM_MAIN_SCREEN_GUI = null;
	...
	public void createIRM() throws RowsExceededException, WriteException,
			BiffException, IOException {

		IrmMainScreenCreator irmMainScreen = IrmMainScreenCreator
				.createIrmMainScreenInstance();
		irmMainScreen.IrmMainScreenCreation();
	...
	public void nodeCreationInPE() {
		...
		for (int i = 0; i < projectNode.getChildCount(); i++) {
			if (((DefaultMutableTreeNode) projectNode.getChildAt(i))
					.getUserObject().toString().equals("IR Analysis")) {
		...
		if (hasCAnalysisNode == true) {
			...
			analysisNode = new DefaultMutableTreeNode("IR Analysis");
	...
	private void getAnalysisNode() {
		...
		String key = "Work Space," + projName + "," + "IR Analysis,";
	...
	private void initializeVariables() {
		...
		DDViewLayer.createMTInstance().mainTabbedFrame.setTitle(" IR Analysis");
