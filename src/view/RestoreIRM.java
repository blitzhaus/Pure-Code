...
public class RestoreIRM {
	...
	public RestoreIRM(String[] pathSplits) {
		...
		LogEntry.createLogEntryInstance().logEntry(
				"Restored Indirect Response Model");
		...
		DDViewLayer.createMTInstance().mainTabbedFrame
				.setTitle("Indirect Response Model Analysis");
		...
		try {
			...
			/*
			 * try { //restoringWB = Workbook.getWorkbook(new
			 * File("ajith.xls")); } catch (BiffException e) {
			 * e.printStackTrace(); }
			 */
	...
	private void restoreLibModelTableDisp(ApplicationInfo readPerst,
		...
		if (linkModelNumber > 0) {
			...
			CaLibModelDispGuiCreator.createCalibraryModelInstance()
					.addImageToDiagramPanel(linkModelNumber);
		}
		...
		for (int i = 0; i < appInfo
			...
			CaResultDispCompCreator.createCaResDispCompInst().completeTextOutputTextArea
					.append(appInfo
							...
							.getIrInfo().getWorkSheetOutputs().getTextoutput()
							.get(i));
