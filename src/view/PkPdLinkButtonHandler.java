...
public class PkPdLinkButtonHandler {
	...
	public void createPkPdLink() throws RowsExceededException, WriteException,
			BiffException, IOException {
		PkPdLinkMainScreenCreator pkpdlinkMainScreen = PkPdLinkMainScreenCreator
				.createPkPdLinkMainScreenInstance();
		pkpdlinkMainScreen.PkPdLinkMainScreenCreation();
	...
	private void getAnalysisNode() {
		...
		String key = "Work Space," + projName + "," + "PKPDLINK Analysis,";
	...
	public void nodeCreationInPE() {
		...
		for (int i = 0; i < projectNode.getChildCount(); i++) {
			if (((DefaultMutableTreeNode) projectNode.getChildAt(i))
					.getUserObject().toString().equals("PKPDLINK Analysis")) {
		...
		if (hasCAnalysisNode == true) {
			...
			analysisNode = new DefaultMutableTreeNode("PKPDLINK Analysis");
