...
public class ReadFile2dStrArray 
{
    ...
    public ReadFile2dStrArray() {
        ...
        while(isFileExists==false)
        {
         ...
         try {
            ...
            System.out.println(" Please enter file to read in the form --> c:/java/file.... ");
         ...
         //  System.out.println(" COLl " + colCount);
           ...
           BufferedReader inp = new BufferedReader (new FileReader (fileName));
            ...
            while (stringToken2.hasMoreTokens()){
                  ...
                  //double t1 = Double.parseDouble(ss);
      ...
      //*******************************************************
		...
		//inputMatrix = convStrArrayToDbl(fileArray, rowCount, colCount);
      }// end while

	//printMatrix("Read Data ..", fileArray, fileArray.length, fileArray[0].length);
	descrArray = loadDescDetails(fileArray);
	//printArray("desc Labels", descrArray, descrArray.length);

	fileArray = filterHeader(fileArray);
	//printMatrix("Filtered Data  ..", fileArray, fileArray.length, fileArray[0].length);

	rowCount = fileArray.length;
	colCount = fileArray[0].length;

	//P2.println("Actual Row Count.."+rowCount);
	//P2.println("Actual Column Count.."+colCount);

	// printArray("rec labels", stringArray, stringArray.length);

	stringArray = filterLabels(stringArray);

	//printArray("filtered record labels", stringArray, stringArray.length);
   ...
   public void printMatrix(String str, String[][] matrix, int rowDim, int colDim)
   {
	...
	public void printMatrix(String str, double[][] matrix, int rowDim, int colDim)
    {
